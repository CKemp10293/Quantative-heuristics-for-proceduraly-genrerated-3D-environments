...
public static class PhotoAnalyser
{
    ...
    public static PhotoMetadata AnalysePhoto(Texture2D smalltex, string id)
    {
        ...
        float maxHueCount = 0;
        if (dominantHues.Count > 0)
        {
            int peakHue = dominantHues[0];
    
            // Sum up the peak hue AND its neighbors (+/- 15 degrees)
            for (int w = -20; w <= 20; w++)
            {
                // Use modulo to wrap around the 360-degree color wheel safely
                int index = (peakHue + w + 360) % 360; 
                maxHueCount += hues[index];
            }
        }
    ...
    private static PhotoMetadata GenerateScores(string id, float avgSat, float contrast, float symmetryDiff,float harmonyRaw, float penaltyRaw)
    {
        ...
        meta.variancePenalty = penaltyRaw;
